  ...
  (cond [(var? e) 
        ...
        [(ifgreater? e)
         (let ([v1 (eval-under-env (ifgreater-e1 e) env)]
           ...
           (if (and (int? v1) (int? v2))
               ...
               (error "e1 and e2 should be numbers in ifgreater")))]
        [(mlet? e)
        ...
        [(call? e)
         (let ([clo (eval-under-env (call-funexp e) env)])
           (if (closure? clo)
               (let* ([act (eval-under-env (call-actual e) env)]
                     [f (closure-fun clo)]
                     [localenv
                      (if (fun-nameopt f)
                          (cons (cons (fun-nameopt f) clo) (cons (cons (fun-formal f) act) (closure-env clo)))
                          (cons (cons (fun-formal f) act) (closure-env clo)))])
                 (eval-under-env (fun-body f) localenv))
